...
export default async function handler(event,res){
  ...
  if(event&&((typeof event.httpMethod==="string")||typeof event.method==="string")){
    ...
    if(url.pathname==="/api/health") { const payload={ok:true,runtime:"netlify",driveConnected:connected,supabaseConnected:Boolean(process.env.SUPABASE_URL&&process.env.SUPABASE_SERVICE_ROLE_KEY)}; return modern?new Response(JSON.stringify(payload),{status:200,headers:{"content-type":"application/json"}}):json(200,payload); }
