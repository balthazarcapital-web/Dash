(function(){
  ...
  function reportHTML(model){
    return `<header class="wr-cover"><small>ABSOLUTTA · GESTÃO DE OBRA</small><h1>${esc(model.title)}</h1><p>Orçado × realizado · Período inteiro</p><p>${model.items.length} itens selecionados · Emitido em ${new Date().toLocaleDateString("pt-BR")}</p></header>
      ...
      ${model.items.map(item=>`<section class="wr-item ${item.actual>item.planned?"over-budget":""}"><small>${esc(item.category)}</small><h2>${esc(item.code)} · ${esc(item.description)}</h2><div class="wr-values"><span>Orçado <b>${money(item.planned)}</b></span><span>Realizado <b>${money(item.actual)}</b></span><span>Saldo <b>${money(item.balance)}</b></span></div><div class="wr-progress ${item.actual>item.planned?"over":""}"><i style="width:${item.planned?Math.max(0,Math.min(100,item.actual/item.planned*100)):0}%"></i></div><p class="wr-note">Material orçado: ${money(item.plannedMaterial)} · Mão de obra orçada: ${money(item.plannedLabor)}</p>
