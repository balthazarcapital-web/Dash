(function(){
  ...
  function renderComparison(){
    ...
    for(const requestItem of items){
      const values=suppliers.map(s=>s.items?.find(item=>item.requestItemId===requestItem.id));const valid=values.filter(item=>item?.unitPrice>0);const best=valid.length?Math.min(...valid.map(item=>item.unitPrice)):0;
      const bestItem=valid.find(item=>item.unitPrice===best);
      html+=`<tr><td><strong>${escapeHtml(requestItem.description)}</strong><small>${escapeHtml(requestItem.unit||"UN")}</small></td><td>${requestItem.quantity}</td>${values.map(item=>item?.unitPrice?`<td class="${item.unitPrice===best?"lowest":""}">${money.format(item.unitPrice)}</td><td>${money.format(itemTotalForRequest(item,requestItem))}</td>`:'<td class="missing">—</td><td class="missing">—</td>').join("")}<td class="best-col lowest">${best?money.format(best):"—"}</td><td class="best-col">${bestItem?money.format(itemTotalForRequest(bestItem,requestItem)):"—"}</td></tr>`;
    }
  ...
  function bind(){
    ...
    document.addEventListener("input",event=>{
      ...
      if(event.target.dataset.supplierField){const field=event.target.dataset.supplierField;supplier[field]=event.target.type==="checkbox"?event.target.checked:["freight","otherCharges","discount"].includes(field)?toNumber(event.target.value):event.target.value;scheduleSave();return}
