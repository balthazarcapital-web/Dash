(function(){
  ...
  function filterRows(rows,module,start,end){return rows.filter(r=>{const d=iso(module==="rentals"?r.sent:r.issue);return !d||(!start||d>=start)&&(!end||d<=end)})}
  let dismiss;
  function open(module,rows,period="Todos os registros",options={}){
    ...
    function generate(){
      ...
      const selected=filterRows(source,module,start,end),label=start||end?(start?formatDate(start):"Início")+" a "+(end?formatDate(end):"Hoje")+" · sem data incluídos":period;
      const m=module==="rentals"?rentalModel(selected,label):fiscalModel(selected,label);
      preview.innerHTML=render(m,options);overlay.querySelector("[data-count]").textContent=selected.length+" registros";return true;
    }
    ...
    const keyboard=e=>{
      ...
      if(e.key==="Tab"){const nodes=[...overlay.querySelectorAll("button,input,a[href]")].filter(n=>!n.disabled),first=nodes[0],last=nodes.at(-1);if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus()}}
    };
    dismiss=close;document.addEventListener("keydown",keyboard,true);
    form.onsubmit=e=>{e.preventDefault();generate();};
    overlay.addEventListener("click",e=>{
      if(e.target===overlay||e.target.closest("[data-close]"))close();
      if(e.target.closest("[data-reset]")){form.reset();generate();}
